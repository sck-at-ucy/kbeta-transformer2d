...
def _parse_cli() -> argparse.Namespace:
    ...
    p.add_argument("config", metavar="YAML", type=str,
    ...
    p.add_argument("--viz", action="store_true",
                   help="Run optional visualisation at the end")
...
def build_config() -> Dict[str, Any]:
    ...
    if ARGS.epochs    is not None: cfg["model_params"]["epochs"]   = ARGS.epochs
...
from .io_utils import (setup_save_directories, setup_load_directories,
                   save_model_and_optimizer, load_model_and_optimizer)
from .data import (initialize_geometry_and_bcs, generate_datasets,
                   save_datasets, load_datasets, data_loader_2D)
...
from .utils import (compare_datasets, print_fresh_run_config,
                    convert_lists_to_tuples, compare_list_states, compare_dict_states, print_config_comparison)

from .plot_utils import (save_distribution_violin_plot, save_distribution_density_heatmap, plot_regressive_predictions_2D,
            plot_predictions_2D, plot_mse_evolution)
...
def run_from_config(cfg: Dict[str, Any]) -> None:
    ...
    if not config["start_from_scratch"]:  # Reload from a previous checkpoint
        ...
        print(load_dir_path)
        (start_epoch, loaded_optimizer_state, loaded_random_state,
        loaded_parameters, loaded_config) = load_model_and_optimizer(model,
                                                                    optimizer, mx_random_state,
                                                                    load_dir_path,
                                                                    model_base_file_name,
                                                                    optimizer_base_file_name,
                                                                    hyper_base_file_name,
                                                                    checkpoint_epoch,comparison=config["compare_current_loaded"])
        if loaded_config:
    ...
    train_step, evaluate_step, state = make_train_and_eval_steps(
        model, optimizer, loss_fn_2D, n_initial,
    ...
    evaluate_step, config, training_data_mlx, training_alphas_mlx, training_dts_mlx, validation_data_mlx,
    ...
    save_dir_path, dataset_save_dir_path, frameplots_save_dir_path, inference_mse_dir_path = setup_save_directories(run_name)
    ...
    evaluate_model(model, evaluate_step, data_loader_2D, test_data_mlx, test_alphas_mlx, test_dts_mlx, config["geometry"]["dx"],
    ...
    if config["model_params"]["mask_type"] == "causal":
        ...
        average_mse = evaluate_self_regressive_model_BeyondL(model, data_loader_2D, test_data_mlx, test_alphas_mlx, test_dts_mlx,
                                               config["geometry"]["dx"],
                                               config["geometry"]["dy"], config["model_params"]["batch_size"],
                                               config["model_params"]["time_steps"], n_replace, n_initial,
                                               output_dir_regress=inference_mse_dir_path)
                                               
        plot_mse_evolution(average_mse, output_dir=inference_mse_dir_path, label="causal")
        ...
        if plots_cfg.get("movie_frames", False):
            plot_regressive_predictions_2D(model, data_loader_2D, {'data': test_data_mlx, 'alphas': test_alphas_mlx,
                                                    'solution_dts': test_dts_mlx, 'batch_size': config["model_params"]["batch_size"],
                                                                   'shuffle': True},
                            n_replace=n_replace, n_initial=n_initial,
        ...
        average_mse = evaluate_model_block_sequence(model, data_loader_2D, test_data_mlx, test_alphas_mlx, test_dts_mlx,
                                      config["geometry"]["dx"],
                                      config["geometry"]["dy"], config["model_params"]["batch_size"],
                                      config["model_params"]["time_steps"],
                                      n_initial,
                                      output_dir_block=inference_mse_dir_path)
        plot_mse_evolution(average_mse, output_dir=inference_mse_dir_path, label="block")
    ...
    if io_and_plots.get("model_saving", False):
        save_model_and_optimizer(model, optimizer, mx_random_state, config, config["model_params"]["epochs"], save_dir_path, model_base_file_name,
