...
# 1) tiny utility ------------------------------------------------------
# ---------------------------------------------------------------------
def _cosine_then_const(init: float, target: float,
                       ramp_steps: int) -> optim.Schedule:
    """
...
# ---------------------------------------------------------------------
def initialize_model_and_optimizer(cfg: Dict[str, Any],
                                   nx: int, ny: int
    ...
    model = HeatDiffusionModel(
        ny, nx,
        p["time_steps"], p["num_heads"], p["num_encoder_layers"],
        p["mlp_dim"],      p["embed_dim"],
        p["start_predicting_from"], p["mask_type"],
    )
    ...
    if name == "adam95":
        optimizer = optim.Adam(
            learning_rate=lr_schedule, betas=[0.90, 0.95],
            eps=1e-8, bias_correction=True)
        _pretty_print_adam("ADAM95", optimizer)

    elif name == "adam999":
        optimizer = optim.Adam(
            learning_rate=lr_schedule, betas=[0.90, 0.999],
            eps=1e-8, bias_correction=True)
        ...
        raise ValueError(f"Unknown optimiser '{name}'. "
                         "Valid: adam95 | adam999 | kourkoutas")
...
def _pretty_print_adam(label: str, opt: optim.Adam) -> None:
    print(f"{label}  β1,β2={opt.betas} | eps={opt.eps:.2e} | "
